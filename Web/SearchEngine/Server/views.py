...
def getDirectoryBrowserInfo(request):
    ...
    result = plugins.getDirectoryBrowserInfo(
        base_dir) if base_dir else plugins.getDirectoryBrowserInfo()
    return result
...
def getHDFSDirectoryBrowserInfo(request):
    base_dir = request.POST.get("currentPath", "")
    result = plugins.getHDFSBrowserInfo(
        base_dir) if base_dir else plugins.getHDFSBrowserInfo()
    return HttpResponse(result)
...
def api_getDirectoryBrowserInfo(request):
    ...
    if request.method == 'POST':
        base_dir = request.POST.get("currentPath", "")
        result = plugins.api_getDirectoryBrowserInfo(
            base_dir) if base_dir else plugins.api_getDirectoryBrowserInfo()
        return HttpResponse(json.dumps(result), content_type='application/json')
    else:
        return HttpResponse(json.dumps(result), content_type='application/json')
...
def api_multiIndexSearch(request):
    result = {'code': 1, 'message': 'Only Support POST Method'}
    if request.method == 'POST':
        searchString1 = request.POST.get(
            "FirstSearchString", "").encode("utf-8")
        searchString2 = request.POST.get(
            "SecondSearchString", "").encode("utf-8")
...
def api_multiIndexSearchStatus(request):
    result = {'code': 1, 'message': 'Only Support POST Method'}
    if request.method == 'POST':
        searchStringList = json.loads(
            request.POST.get("searchStringList", '[]'))
        relation = request.POST.get("relation", "")
        if len(searchStringList) == 0 or relation == "":
            result['message'] = 'Wrong Relation or SearchString Input'
        else:
            result = plugins.api_multiIndexSearchStatus(
                searchStringList, relation)
        return HttpResponse(json.dumps(result), content_type='application/json')
    else:
        return HttpResponse(json.dumps(result), content_type='application/json')
...
def api_initFileContent(request):
    ...
    if request.method == 'POST':
        ...
        itemNum = 20
        print('[REQUEST]\n\t_uri:{}\n\t_matches:{}\n\tip:{}'.format(
            _uri, _matches, _node_ip))
        result = plugins.api_initFileContent(_uri, _matches, itemNum)
        return HttpResponse(json.dumps(result), content_type='application/json')
    else:
        return HttpResponse(json.dumps(result), content_type='application/json')
...
def api_addNodeInfo(request):
    ...
    if request.method == 'POST':
        ...
        hostDefaultDirectory = request.POST.get(
            'defaultDirectory', '/dataFile')
        if plugins.checkHostIP(hostIP):
            result = plugins.api_addNodeInfo(
                hostIP, hostName, hostIsAlive, hostDefaultDirectory)
        else:
...
def fileContentReader(request):
    ...
    if request.method == 'POST':
        ...
        params = {"fileName": _uri, "matches": json.dumps(
            theMatches, ensure_ascii=False), "pageNum": pageNum, "itemNum": itemNum, "fileLength": fileLength}
...
def api_binarySearchResult(request):
    ...
    if request.method == 'POST':
        ...
        if scale == 0 or search_string == "":
            ...
            pay_load = {'search_string': search_string, 'scale': scale, 'pageNum': pageNum,
                        'pageSize': pageSize, 'order': order, 'expandedName': expandedName}
...
def api_binarySearch(request):
    ...
    if request.method == 'POST':
        ...
        if not keyword:
            ...
            result['message'] = u'[参数缺失] No keyword'
            return HttpResponse(json.dumps(result), content_type='application/json')
        ...
        if r_search['code'] == 0:
            ...
            result['message'] = r_search['message']
    else:
...
@csrf_exempt
def api_binarySearchDetail(request):
    result = {'resultCode': 0, 'message': u'响应成功', 'data': []}
    if request.method == 'GET':
        ...
        binaryType = int(request.GET.get('binaryType', 0))
        extention = request.GET.get('extention', '')
        ...
        if not filePath:
            result['resultCode'] = 2003
            result['message'] = u'[参数缺失] No filePath'
            return HttpResponse(json.dumps(result), content_type='application/json')
        if not pageNum > 0:
            result['resultCode'] = 2003
            result['message'] = u'[参数错误] pageNum should greater than 1'
        ...
        # Get Cache Search Formated Result
        pay_load = {'search_string': search_string, 'filePath':filePath,'scale': scale, 
                'pageNum': pageNum, 'pageSize': pageSize, 'extention': extention}
        r_search_detail = plugins.api_binarySearchDetail(pay_load)
        if r_search_detail['code']==0:
            result['resultCode'] = 1000
            result['data'] = [r_search_detail['message']]
        else:
            result['resultCode'] = 2001
            result['message'] = r_search_detail['message']
    else:
        result['resultCode'] = 2001
        result['message'] = u'[服务器错误] HTTP方法不支持'
    return HttpResponse(json.dumps(result), content_type='application/json')
